...
move_source(){
    mkdir -p output
    find -type f  | grep -v "output"| grep ".ymp$" | sort -V | while read line ; do
        mv $line output
    done
}
find -type f | grep ympbuild | sort -V | while read line ; do
    ...
    if ! find ./output/ -type f | grep "${name}/${name}_" | grep ".ymp$" &>/dev/null ; then
        echo -e "\033[33;1m=>> SOURCE BUILD START:\033[;0m $line"
        ymp --sandbox --shared="$path" build "$path" --ignore-dependency --use=all --allow-oem $@ --verbose || exit 1
        move_source
